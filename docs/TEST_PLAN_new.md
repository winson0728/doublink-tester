...
|----------|------|
| v1.0 | 初版，52 個測項 |
| v1.1 | TCP 劣化測試擴展為 3 種 ATSSS 模式各別量測（+14 項）；補充 Failover 章節（+8 項）；總計 74 個測項 |
...
本 Test Plan 涵蓋 Doublink ATSSS（Access Traffic Steering, Switching, Splitting）Multilink 系統的自動化驗證測試，共 **74 個測項**，分成 4 大功能群組。
...
| B | Degradation | 49 | Profile 套用驗證、TCP/UDP 劣化量測（TCP × 3 模式）、Steering、Recovery、Failover |
...
### B-11 ~ B-13｜TCP 基準量測（× 3 ATSSS 模式，共 3 項）

> **設計說明：** TCP 基準測試對三種 ATSSS 模式各別執行一次，水平比較三種模式的基準效能。

| Test ID | Test Item | ATSSS 模式 | Test Condition | Expected Result |
|---------|-----------|----------|---------------|-----------------|
| **B-11** | TCP 基準 [real_time] | `real_time` | 無 NetEmu 限速 | 吞吐量 > 0 Mbps |
| **B-12** | TCP 基準 [bonding] | `bonding` | 無 NetEmu 限速 | 吞吐量 > 0 Mbps |
| **B-13** | TCP 基準 [duplicate] | `duplicate` | 無 NetEmu 限速 | 吞吐量 > 0 Mbps |

| 共用設定 | 說明 |
|------|------|
| **Test Setup** | 切換對應 ATSSS 模式 → 不套用 NetEmu 規則 → iperf3 TCP 10s，parallel=4 |
| **實測參考值** | real_time ≈ 349 Mbps，bonding ≈ 329 Mbps，duplicate ≈ 125 Mbps（duplicate 雙路傳輸相同資料，非聚合） |

---

### B-14 ~ B-31｜TCP 劣化條件量測（6 條件 × 3 模式 = 18 項）

> **設計說明：** 每個劣化條件在三種 ATSSS 模式下各別執行，量測不同模式在相同劣化環境的吞吐量差異。

| Test ID | Test Item | ATSSS 模式 | Test Condition | Expected Result |
|---------|-----------|----------|---------------|-----------------|
| **B-14** | TCP [real_time] — symmetric_mild_loss | `real_time` | 雙線 50M/20ms/**0.3% loss** | ≥ 2.0 Mbps |
| **B-15** | TCP [bonding] — symmetric_mild_loss | `bonding` | 雙線 50M/20ms/**0.3% loss** | ≥ 2.0 Mbps |
| **B-16** | TCP [duplicate] — symmetric_mild_loss | `duplicate` | 雙線 50M/20ms/**0.3% loss** | ≥ 2.0 Mbps |
| **B-17** | TCP [real_time] — symmetric_mild_latency | `real_time` | 雙線 40M/**60ms**/8ms jitter | ≥ 1.0 Mbps |
| **B-18** | TCP [bonding] — symmetric_mild_latency | `bonding` | 雙線 40M/**60ms**/8ms jitter | ≥ 1.0 Mbps |
| **B-19** | TCP [duplicate] — symmetric_mild_latency | `duplicate` | 雙線 40M/**60ms**/8ms jitter | ≥ 1.0 Mbps |
| **B-20** | TCP [real_time] — congested_recoverable | `real_time` | 雙線 **10M/80ms/1% loss** | ≥ 0.5 Mbps |
| **B-21** | TCP [bonding] — congested_recoverable | `bonding` | 雙線 **10M/80ms/1% loss** | ≥ 0.5 Mbps |
| **B-22** | TCP [duplicate] — congested_recoverable | `duplicate` | 雙線 **10M/80ms/1% loss** | ≥ 0.5 Mbps |
| **B-23** | TCP [real_time] — 5g_degraded_moderate | `real_time` | 5G **20M/60ms/1.5%**，WiFi 40M | ≥ 1.0 Mbps |
| **B-24** | TCP [bonding] — 5g_degraded_moderate | `bonding` | 5G **20M/60ms/1.5%**，WiFi 40M | ≥ 1.0 Mbps |
| **B-25** | TCP [duplicate] — 5g_degraded_moderate | `duplicate` | 5G **20M/60ms/1.5%**，WiFi 40M | ≥ 1.0 Mbps |
| **B-26** | TCP [real_time] — wifi_degraded_moderate | `real_time` | WiFi **20M/40ms/1.5%**，5G 40M | ≥ 1.0 Mbps |
| **B-27** | TCP [bonding] — wifi_degraded_moderate | `bonding` | WiFi **20M/40ms/1.5%**，5G 40M | ≥ 1.0 Mbps |
| **B-28** | TCP [duplicate] — wifi_degraded_moderate | `duplicate` | WiFi **20M/40ms/1.5%**，5G 40M | ≥ 1.0 Mbps |
| **B-29** | TCP [real_time] — asymmetric_mixed_moderate | `real_time` | 5G 40M/80ms，WiFi **40M/20ms/1.5%** | ≥ 1.0 Mbps |
| **B-30** | TCP [bonding] — asymmetric_mixed_moderate | `bonding` | 5G 40M/80ms，WiFi **40M/20ms/1.5%** | ≥ 1.0 Mbps |
| **B-31** | TCP [duplicate] — asymmetric_mixed_moderate | `duplicate` | 5G 40M/80ms，WiFi **40M/20ms/1.5%** | ≥ 1.0 Mbps |

| 共用設定 | 說明 |
|------|------|
| **Test Setup** | 切換 ATSSS 模式 → 套用對應 profile（4 條 NetEmu 規則）→ 等待 5s → iperf3 TCP 10s，parallel=4 |
| **Test Purpose** | 量測三種 ATSSS 模式在相同劣化條件下的吞吐量差異（橫向比較） |
| **Allure 標題格式** | `TCP throughput — 5g_degraded_moderate [bonding]`；JSON 附件含 `"mode"` 欄位 |
...
### B-33 ~ B-36｜UDP 劣化條件量測

| Test ID | Test Item | Test Condition | Expected Result |
|---------|-----------|---------------|-----------------|
| **B-33** | UDP — symmetric_mild_loss | 雙線 50M / 20ms / **0.3% loss** | UDP > 0 Mbps；loss ≤ **10%**（若 ≤100%） |
| **B-34** | UDP — symmetric_mild_latency | 雙線 40M / **60ms** / 8ms jitter | UDP > 0 Mbps；loss ≤ **60%**（若 ≤100%） |
| **B-35** | UDP — wifi_interference_moderate | WiFi 35M/25ms/0.4% + 動態變化 | UDP > 0 Mbps；loss ≤ **10%**（若 ≤100%） |
| **B-36** | UDP — asymmetric_mixed_moderate | 5G 40M/80ms，WiFi 40M/20ms/1.5% | UDP > 0 Mbps；loss ≤ **10%**（若 ≤100%） |

> **注意：** loss_pct > 100% 時，代表 ATSSS duplicate 模式送出重複封包，iperf3 計算失真，此情況僅驗證吞吐量 > 0，不驗證 loss。

---

### B-37 ~ B-40｜ATSSS 路由導向（Steering）驗證

**Test Purpose：** 當單一 link 劣化時，ATSSS 應自動將流量導向健康的 link，確保整體吞吐量不因單線故障而崩潰。

| Test ID | Test Item | Test Condition | Expected Result | 實測參考值 |
|---------|-----------|---------------|-----------------|-----------|
| **B-37** | Steering — 5G 劣化導向 WiFi | 5G 20M/60ms/1.5%，WiFi **40M/10ms** 健康 | TCP ≥ **1.0 Mbps** | 31.8 Mbps |
| **B-38** | Steering — WiFi 劣化導向 5G | WiFi 20M/40ms/1.5%，5G **40M/20ms** 健康 | TCP ≥ **1.0 Mbps** | 27.8 Mbps |
| **B-39** | Steering — 5G 高延遲導向 WiFi | 5G **50M/100ms**，WiFi **50M/10ms** 正常 | TCP ≥ **1.0 Mbps** | 36.2 Mbps |
| **B-40** | Steering — WiFi 高延遲導向 5G | WiFi **50M/100ms**，5G **50M/10ms** 正常 | TCP ≥ **1.0 Mbps** | 38.3 Mbps |
...
### B-42 ~ B-49｜Failover 測試（Link Disconnect / Intermittent）

**執行檔：** `tests/test_degradation/test_failover.py`

**驗證目的：** 模擬 LINE A（5G）或 LINE B（WiFi）完全斷線或間歇斷線，確認 ATSSS 能自動切換至存活線路維持流量。

---

#### B-42 ~ B-43｜Link 完全斷線 Failover

| 欄位 | 內容 |
|------|------|
| **Test Item** | 5G 斷線（B-42）/ WiFi 斷線（B-43）後流量自動切換至存活線路 |
| **Test Purpose** | 驗證 ATSSS bonding 模式在一條線路 100% loss 時，能透過另一條線路繼續傳輸 |
| **Test Setup** | 設定 bonding 模式 → 量測 baseline 5s → 套用斷線 profile（100% loss）→ 量測 failover 期間 10s |
| **Test Condition** | `5g_disconnect_visible`：5G 下行/上行 100% loss，WiFi 正常；`wifi_disconnect_visible`：反之 |
| **Expected Result** | failover 期間吞吐量 > 0 Mbps（流量透過存活線路繼續傳輸） |

---

#### B-44 ~ B-45｜各模式下 5G 斷線 Failover

| Test ID | Test Item | ATSSS 模式 | Expected Result |
|---------|-----------|----------|-----------------|
| **B-44** | bonding 模式下 5G 斷線 failover | `bonding` | 吞吐量 > 0 Mbps（WiFi 存活） |
| **B-45** | duplicate 模式下 5G 斷線 failover | `duplicate` | 吞吐量 > 0 Mbps（WiFi 存活） |

| 共用設定 | 說明 |
|------|------|
| **Test Setup** | 設定對應 ATSSS 模式 → 套用 5g_disconnect_visible（5G 100% loss）→ iperf3 TCP 10s |
| **Test Purpose** | 比較 bonding 與 duplicate 模式在 5G 完全斷線時的 failover 行為 |

---

#### B-46 ~ B-47｜間歇斷線（Intermittent Disconnect）存活驗證

| Test ID | Test Item | 閃斷線路 | Expected Result |
|---------|-----------|----------|-----------------|
| **B-46** | 5G 間歇斷線（每 15s 斷 2s × 10 次） | 5G (LINE A) | 吞吐量 > 0 Mbps |
| **B-47** | WiFi 間歇斷線（每 15s 斷 2s × 10 次） | WiFi (LINE B) | 吞吐量 > 0 Mbps |

| 共用設定 | 說明 |
|------|------|
| **Test Setup** | 設定 duplicate 模式 → 套用間歇斷線 profile → iperf3 TCP 20s，parallel=2（覆蓋至少 2 次斷線週期） |
| **Test Purpose** | 驗證 ATSSS duplicate 模式在間歇性斷線（flapping）時透過冗餘路徑維持吞吐量 |

---

#### B-48｜透過 NetEmu API 即時排程斷線

| 欄位 | 內容 |
|------|------|
| **Test Item** | iperf3 執行中透過 NetEmu schedule_disconnect API 排程 3 秒斷線 |
| **Test Purpose** | 驗證 API 排程斷線功能可在流量傳輸過程中即時生效，且 duplicate 模式透過 WiFi 維持連續性 |
| **Test Setup** | 設定 duplicate 模式 → 啟動 iperf3 TCP 20s → 第 5s 呼叫 `schedule_disconnect(line_a_dl, 3s)` → 等待 iperf3 完成 |
| **Test Condition** | LINE A DL 排程 3 秒斷線；duplicate 模式保護 |
| **Expected Result** | iperf3 完整完成，吞吐量 > 0 Mbps |
...
| 欄位 | 內容 |
|------|------|
| **Test Item** | 5G 斷線後清除規則，吞吐量應恢復至斷線前水平 |
| **Test Purpose** | 驗證 NetEmu 規則清除（模擬線路恢復）後，ATSSS 能重新利用 5G 路徑提升吞吐量 |
| **Test Setup** | 設定 bonding 模式 → 套用 5g_disconnect_visible → 量測斷線中吞吐量（5s）→ 清除規則 → 等待 5s → 量測恢復後吞吐量（5s） |
| **Test Condition** | 5G 斷線中（100% loss）→ 清除規則（5G 恢復） |
| **Expected Result** | 恢復後吞吐量 > 0 Mbps；恢復後 / 斷線中比值 ≥ 0.5 |
...
*文件路徑：`docs/TEST_PLAN.md` | Repo：`github.com/winson0728/doublink-tester` | 最後更新：2026-04-17 (v1.1)*
